...
def add_user_to_db(cursor, db_conn, data):
    query = """
        INSERT INTO users (
            user_ID, full_name, email, password_hash, role, bio,
            ...
            preferred_project_types, created_at, updated_at
        ) VALUES (
    ...
    cursor.execute(query, (
        str(uuid.uuid4()), data["full_name"], data["email"], data["password_hash"], data["role"], data["bio"],
        ...
        data["preferred_project_types"], now, now
    ))
    db_conn.commit()
...
def update_user_in_db(cursor, db_conn, user_id, data):
    query = """
        UPDATE users SET
            ...
            preferred_project_types = %s, updated_at = %s
        WHERE user_ID = %s
    """
    now = datetime.datetime.now()
    cursor.execute(query, (
        ...
        data["preferred_learning_platform"], data["location"], data["available_for_projects"],
        data["programming_languages"], data["tools_and_technologies"], data["interests"],
        data["preferred_project_types"], now, user_id
    ))
    db_conn.commit()
